...
const mybooks = async () => {
  const userdata = await fetch("https://kiran1432.pythonanywhere.com/mybooks", {
    method: "GET",
    headers: {
      "Content-type": "application/json",
      "X-CSRFToken": csrfToken,
...
const borrow = async (id, name, description, author) => {
  ...
  await fetch(`https://kiran1432.pythonanywhere.com/book_update/${id}`, {
    method: "PUT",
...
const owndelete = async () => {
  await fetch(`https://kiran1432.pythonanywhere.com/memberowndelete`, {
    method: "DELETE",
    headers: {
      "Content-type": "application/json",
      "X-CSRFToken": csrfToken,
